...
- ちなみに、`React.cache`によるキャッシュはRSCサーバーへの[リクエストごとに無効化される](https://ja.react.dev/reference/react/cache#caveats)ので、リクエストを跨いで同じインスタンスが使われることはない

> React は、サーバへの各リクエストごとにすべてのメモ化された関数のキャッシュを無効化します。

- むしろ、リクエストを跨いで同じインスタンスが使われると、ユーザーの個人情報などのデータが漏洩するリスクがあるので、やるべきではない
